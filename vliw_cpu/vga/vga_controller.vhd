...
ENTITY vga_controller IS
	GENERIC(
		...
		-- fast sim
		--h_pixels	:	INTEGER := 10;		--horiztonal display width in pixels
		...
		v_pol		:	STD_LOGIC := '1');	--vertical sync pulse polarity (1 = positive, 0 = negative)
			...
			--v_pulse + v_bp + v_pixels + v_fp  
			-- 4  + 23 + 600 + 1 = 628
			
			--h_pulse + h_bp + h_pixels + h_fp
			-- 128 + 88 +800 + 40 = 1056
...
BEGIN
	...
	n_sync <= '1';   --no sync on green
	
	p0: PROCESS(pixel_clk, reset_n)
	...
	BEGIN
		...
		ELSIF(pixel_clk'EVENT AND pixel_clk = '1') THEN
			...
			IF (h_count =  h_period - 1) and  v_count = v_period - 1 then				
				refresh <= '1';											
			ELSE																	
				refresh <= '0';											
			END IF;
