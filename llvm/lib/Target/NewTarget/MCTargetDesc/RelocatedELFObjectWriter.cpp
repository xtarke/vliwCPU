/*
 * RelocatedRelocatedELFObjectWriter.c
 *
 *  Created on: Apr 19, 2013
 *      Author: andreu
 */

//===- lib/MC/RelocatedELFObjectWriter.cpp - ELF File Writer -------------------===//
//
...
namespace {
class RelocatedELFObjectWriter : public MCObjectWriter {
  protected:

    static bool isFixupKindPCRel(const MCAssembler &Asm, unsigned Kind);
    static bool RelocNeedsGOT(MCSymbolRefExpr::VariantKind Variant);
    static uint64_t SymbolValue(RelocatedELFObjectWriter &ELFWriter,
    		MCSymbolData &Data, const MCAsmLayout &Layout);
  ...
  public:
    RelocatedELFObjectWriter(MCELFObjectTargetWriter *MOTW,
                    raw_ostream &_OS, bool IsLittleEndian, unsigned int BaseAddress_)
      : MCObjectWriter(_OS, IsLittleEndian),
        TargetObjectWriter(MOTW),
        NeedsGOT(false), NeedsSymtabShndx(false), BaseAddress(BaseAddress_){
    }

    virtual ~RelocatedELFObjectWriter();
...
bool RelocatedELFObjectWriter::isFixupKindPCRel(const MCAssembler &Asm, unsigned Kind) {
...
bool RelocatedELFObjectWriter::RelocNeedsGOT(MCSymbolRefExpr::VariantKind Variant) {
  switch (Variant) {
  default:
    return false;
...
RelocatedELFObjectWriter::~RelocatedELFObjectWriter()
{}

// Emit the ELF header.
void RelocatedELFObjectWriter::WriteHeader(const MCAssembler &Asm,
                                  uint64_t SectionDataSize,
                                  unsigned NumberOfSections) {
  // ELF Header
  // ----------
...
void RelocatedELFObjectWriter::WriteSymbolEntry(MCDataFragment *SymtabF,
                                       MCDataFragment *ShndxF,
                                       uint64_t name,
...
uint64_t RelocatedELFObjectWriter::SymbolValue(RelocatedELFObjectWriter &ELFWriter,
		MCSymbolData &Data,const MCAsmLayout &Layout) {
  ...
  if (Data.getFragment()) {
    if (Data.getFlags() & ELF_Other_ThumbFunc)
      return ELFWriter.BaseAddress + Layout.getSymbolOffset(&Data)+1;
    else
      return ELFWriter.BaseAddress + Layout.getSymbolOffset(&Data);
  }

  return 0;
}

void RelocatedELFObjectWriter::ExecutePostLayoutBinding(MCAssembler &Asm,
                                               const MCAsmLayout &Layout) {
...
void RelocatedELFObjectWriter::WriteSymbol(MCDataFragment *SymtabF,
                                  MCDataFragment *ShndxF,
                                  ELFSymbolData &MSD,
                                  const MCAsmLayout &Layout) {
  ...
  //OrigData.dump();
  //std::cout << "\n"; 
 // std::cout << "Section index: " << MSD.SectionIndex << "\n"; 
  ...
  uint64_t Value = SymbolValue(*this, Data, Layout);
  uint64_t Size = 0;
...
void RelocatedELFObjectWriter::WriteSymbolTable(MCDataFragment *SymtabF,
                                       MCDataFragment *ShndxF,
                                       const MCAssembler &Asm,
                                       const MCAsmLayout &Layout,
                                    const SectionIndexMapTy &SectionIndexMap) {
...
const MCSymbol *RelocatedELFObjectWriter::SymbolToReloc(const MCAssembler &Asm,
                                               const MCValue &Target,
                                               const MCFragment &F,
                                               const MCFixup &Fixup,
                                               bool IsPCRel) const {
...
void RelocatedELFObjectWriter::RecordRelocation(const MCAssembler &Asm,
                                       const MCAsmLayout &Layout,
                                       const MCFragment *Fragment,
                                       const MCFixup &Fixup,
                                       MCValue Target,
...
uint64_t
RelocatedELFObjectWriter::getSymbolIndexInSymbolTable(const MCAssembler &Asm,
                                             const MCSymbol *S) {
  MCSymbolData &SD = Asm.getSymbolData(*S);
  return SD.getIndex();
}

bool RelocatedELFObjectWriter::isInSymtab(const MCAssembler &Asm,
...
bool RelocatedELFObjectWriter::isLocal(const MCSymbolData &Data, bool isSignature,
                              bool isUsedInReloc) {
  if (Data.isExternal())
    return false;
...
void RelocatedELFObjectWriter::ComputeIndexMap(MCAssembler &Asm,
                                      SectionIndexMapTy &SectionIndexMap,
                                      const RelMapTy &RelMap) {
  unsigned Index = 1;
...
void RelocatedELFObjectWriter::ComputeSymbolTable(MCAssembler &Asm,
                                      const SectionIndexMapTy &SectionIndexMap,
                                         RevGroupMapTy RevGroupMap,
                                         unsigned NumRegularSections) {
...
void RelocatedELFObjectWriter::CreateRelocationSections(MCAssembler &Asm,
                                               MCAsmLayout &Layout,
                                               RelMapTy &RelMap) {
...
void RelocatedELFObjectWriter::WriteRelocations(MCAssembler &Asm, MCAsmLayout &Layout,
                                       const RelMapTy &RelMap) {
  for (MCAssembler::const_iterator it = Asm.begin(),
         ie = Asm.end(); it != ie; ++it) {
    const MCSectionData &SD = *it;
...
void RelocatedELFObjectWriter::WriteSecHdrEntry(uint32_t Name, uint32_t Type,
...
void RelocatedELFObjectWriter::WriteRelocationsFragment(const MCAssembler &Asm,
                                               MCDataFragment *F,
                                               const MCSectionData *SD) {
...
void RelocatedELFObjectWriter::CreateMetadataSections(MCAssembler &Asm,
                                             MCAsmLayout &Layout,
                                             SectionIndexMapTy &SectionIndexMap,
                                             const RelMapTy &RelMap) {
  MCContext &Ctx = Asm.getContext();
...
void RelocatedELFObjectWriter::CreateIndexedSections(MCAssembler &Asm,
                                            MCAsmLayout &Layout,
                                            GroupMapTy &GroupMap,
                                            RevGroupMapTy &RevGroupMap,
                                            SectionIndexMapTy &SectionIndexMap,
...
void RelocatedELFObjectWriter::WriteSection(MCAssembler &Asm,
                                   const SectionIndexMapTy &SectionIndexMap,
                                   uint32_t GroupSymbolIndex,
                                   uint64_t Offset, uint64_t Size,
                                   uint64_t Alignment,
  ...
  WriteSecHdrEntry(SectionStringTableIndex[&Section], Section.getType(),
                   Section.getFlags(), BaseAddress, Offset, Size, sh_link, sh_info,
                   Alignment, Section.getEntrySize());
}

bool RelocatedELFObjectWriter::IsELFMetaDataSection(const MCSectionData &SD) {
  return SD.getOrdinal() == ~UINT32_C(0) &&
    !SD.getSection().isVirtualSection();
}

uint64_t RelocatedELFObjectWriter::DataSectionSize(const MCSectionData &SD) {
  uint64_t Ret = 0;
...
uint64_t RelocatedELFObjectWriter::GetSectionFileSize(const MCAsmLayout &Layout,
                                             const MCSectionData &SD) {
...
uint64_t RelocatedELFObjectWriter::GetSectionAddressSize(const MCAsmLayout &Layout,
                                                const MCSectionData &SD) {
  if (IsELFMetaDataSection(SD))
    return DataSectionSize(SD);
...
void RelocatedELFObjectWriter::WriteDataSectionData(MCAssembler &Asm,
                                           const MCAsmLayout &Layout,
                                           const MCSectionELF &Section) {
...
void RelocatedELFObjectWriter::WriteSectionHeader(MCAssembler &Asm,
  ...
  WriteSecHdrEntry(0, 0, 0, BaseAddress, 0, FirstSectionSize, FirstSectionLink, 0, 0, 0);
...
void RelocatedELFObjectWriter::ComputeSectionOrder(MCAssembler &Asm,
                                  std::vector<const MCSectionELF*> &Sections) {
  for (MCAssembler::iterator it = Asm.begin(),
         ie = Asm.end(); it != ie; ++it) {
    const MCSectionELF &Section =
...
void RelocatedELFObjectWriter::WriteObject(MCAssembler &Asm,
                                  const MCAsmLayout &Layout) {
  GroupMapTy GroupMap;
  RevGroupMapTy RevGroupMap;
  SectionIndexMapTy SectionIndexMap;
  ...
  for (unsigned i = 0; i < NumRegularSections + 1; ++i) {
    const MCSectionELF &Section = *Sections[i];
    //std::cout << "name: " << Section.getSectionName().data() << "\n";
    const MCSectionData &SD = Asm.getOrCreateSectionData(Section);
...
bool
RelocatedELFObjectWriter::IsSymbolRefDifferenceFullyResolvedImpl(const MCAssembler &Asm,
                                                      const MCSymbolData &DataA,
                                                      const MCFragment &FB,
                                                      bool InSet,
                                                      bool IsPCRel) const {
...
MCObjectWriter *llvm::createRelocatedELFObjectWriter(MCELFObjectTargetWriter *MOTW,
                                            raw_ostream &OS,
                                            bool IsLittleEndian, unsigned int BaseAddress) {
  return new RelocatedELFObjectWriter(MOTW, OS, IsLittleEndian, BaseAddress);
}
