...
void newtarget::Link::ConstructJob(Compilation &C, const JobAction &JA,
                                      const InputInfo &Output,
                                      const InputInfoList &Inputs,
                                      const ArgList &Args,
                                      const char *LinkingOutput) const {
  ...
  if (Output.isFilename()) {
    CmdArgs.push_back(Args.MakeArgString(std::string("-o") + Output.getFilename()));
  } else {
    assert(Output.isNothing() && "Invalid output.");
  }
  
  if(Inputs.size() > 1){
	  assert(true && "Only one file is allowed as input!");
  } 
  
  InputInfoList::const_iterator it = Inputs.begin();
  
  if(it->isFilename()){
	  CmdArgs.push_back(Args.MakeArgString(std::string("-e") + it->getFilename())); 
  } 	   

  DerivedArgList &args = C.getArgs();
  
  Arg *InputSourceArg = args.getLastArg(options::OPT_INPUT);
  
  CmdArgs.push_back(Args.MakeArgString(std::string("-g") + InputSourceArg->getSpelling() + ".cfg"));
  

  const char *Exec =
    Args.MakeArgString(getToolChain().GetProgramPath("wcet"));
  C.addCommand(new Command(JA, *this, Exec, CmdArgs));
}
