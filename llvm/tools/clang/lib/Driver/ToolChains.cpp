...
NewTarget::NewTarget(const Driver &D, const llvm::Triple& Triple)
  : ToolChain(D, Triple) {
	  ...
	  if(!WcetDir){
		...
		llvm::errs() << WcetDir << "\n";
		getProgramPaths().push_back(WcetDir);
	  }	  
}

Tool &NewTarget::SelectTool(const Compilation &C, const JobAction &JA,
                          const ActionList &Inputs) const {
  Action::ActionClass Key;
  if (getDriver().ShouldUseClangCompiler(C, JA, getTriple()))
    Key = Action::AnalyzeJobClass;
  ...
  if (!T) {
    ...
    case Action::LinkJobClass:
      ...
      T = new tools::newtarget::Link(*this); break;
    }
  }
